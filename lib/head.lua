...
do
	...
	--	new_env.pwd = string.sub(boot_file, 1, -#segs[#segs] - 2)
		...
		local str = [[
			...
			-- Modified by KutupTilkisi for Dev Env
			-- Apps should be able to access kernel functions
	...
	--	setmetatable(getmetatable("").__index, {__index=function(s, i) return pod(i) end})
		...
		-- Modified by KutupTilkisi for Dev Env
		-- Includes any library on system lib.
		-- TODO: Maybe make it so that it bundles system libs with save.
		-- This way anyone can open the file on any system.
		function require(filename)
			local file = fullpath(filename)

			if not fetch(file) then
				-- if file is not on same directory. Possible system lib.
				file = "/system/lib/"..filename
				if not fetch(file) then
			...
			local func,err = load(src, "@"..file, "t", _ENV)
	...
	--[[
			...
			-- Modified by KutupTilkisi for Dev Env
			-- It is better to return result of func() as that is how lua does.
			-- Maybe remove true in future? Has no usebility.
			func()
